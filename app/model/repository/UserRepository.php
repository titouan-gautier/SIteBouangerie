<?php
    ...
    public function __construct() {
        ...
        $this->connexion = SPDO::getInstance($dsn,
        CFG["db"]["login"],CFG["db"]["password"], CFG["db"]["options"],CFG["db"]["exec"])
        ->getConnexion();
    }
    ...
    public function findByPseudo(string $pseudo): ?UserEntity {
        $statement = $this->connexion->prepare("select * from user where pseudo=:pseudo");
    ...
    public function update(string $pseudoS, UserEntity $user): ?UserEntity {
        ...
        try{
            $statement = $this->connexion->prepare("update user set pseudo=:pseudo, password=:password, status=:status where pseduo=:pseudoS");
